...
BMPEditer::BMPEditer(const string& file) : BMP(file)
{

}


void BMPEditer::BoundingRect(const char* fileName)
{
...
void BMPEditer::TrimBoundingRect(const char* fileName) 
{
...
void BMPEditer::editFileStructure(File_Structure& fileStructure, Position_Area area, int padRowSize)
{
	vector<vector<int> > originalPixelArray = this->GetPixelArray();
	//edit the image data
	vector<char> newImageData;
...
int BMPEditer::calculatePadRowSize(UINT startX, UINT endX)
{
	return (endX - startX) % 4;
}

void BMPEditer::CreateNewBMP(File_Structure file_structure, const char * fileName)
{
	FILE* outputFile;
...
void BMPEditer::updateImageData(vector<vector<int> >& newPixelArray, vector<char>& newImageDate, Position_Area area, int padRowSize)
{
...
void BMPEditer::editeThePixelArray(vector<vector<int> >& originalPixelArray, Position_Area area)
{
	for (UINT positionX = area.startX; positionX < area.endX; positionX++)
	{
		for (UINT positionY = area.startY; positionY < area.endY; positionY++)
...
void BMPEditer::findTheUniquePosBT(vector<vector<int> >& originalPixelArray, Position_Area area, Position& bottomPos, Position& topPos)
{
...
void BMPEditer::findTheUniquePosLR(vector<vector<int> >& originalPixelArray, Position_Area area, Position& leftPos, Position& rightPos)
{
	bool leftFlag = false;
...
void BMPEditer::writeHeaders(ofstream& file) {
